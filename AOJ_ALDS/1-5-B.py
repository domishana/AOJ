...
def merge(a: list, left: int, middle: int, right: int) -> int:
    n1 = middle - left
    n2 = right - middle

    left_list = [a[left + i] for i in range(n1)]
    right_list = [a[middle + i] for i in range(n2)]
...
def merge_sort(a: list, left: int, right: int, counter: int) -> (list, int):
    if left+1 < right:
        mid = (left + right)//2
